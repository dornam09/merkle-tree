...
Node *newNode(unsigned char *x) {
	Node *pnt = (Node *) malloc (sizeof(Node)); /* allocates physical memory for the node */
	memcpy(pnt->n, x, 20); /* inserts the information received as input in the field(s) in the list */
...
Node *preInsert(Node *top, unsigned char *x) {
...
Node *orderInsert(Node *top, unsigned char *x) {
	if (top == NULL || top -> n > x) {
...
Node *postInsert(Node *top, unsigned char *x) {
	if (top == NULL)
