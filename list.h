...
typedef struct node {
	unsigned char n[20];              /* data field(s) */
	/* float b;
	 * char c;
	 * ... etc.
	 */
...
Node *newNode(unsigned char * ); /* physically creates a new node */
...
Node *preInsert(Node *, unsigned char * ); /* inserts a new item at the top of the list */

Node *orderInsert(Node *, unsigned char * ); /* inserts a new element in order, according to a key field */

Node *postInsert(Node *, unsigned char * ); /* inserts a new item at the end of the list */
